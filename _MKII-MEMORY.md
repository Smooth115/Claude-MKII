...
| 2026-03-27 | Vault ACCESS_GATE fix (MK2_PHANTOM) | User called MK2_PHANTOM from inside vault via commit bfea0f5 + task description. ACCESS_GATE codename auth was too rigid: required exact string "mk2-phantom", no owner bypass, no fuzzy matching for phone typos. User couldn't get agents to respond from vault. | Fixed ACCESS_GATE.md: added owner unconditional access section, fuzzy codename matching (accepts MK2_PHANTOM/MK_2-PHANTUM/etc), added Condition 3 (user-presence = authorization). Gate now protects against agents, not the owner. |
