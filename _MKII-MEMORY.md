...
| 2026-03-27 | MK2_PHANTOM_KEY revocation — user action | User created `MK2_PHANTOM_KEY` as an environment secret, then deleted it ~32 minutes later. Same session as issue #59 (video showing suspicious package imports / failed network requests). Repo secret `MK2_PHANTOM_TOKEN` (different name, powers phantom workflows) still exists — unaffected. Repo-level `MK2_PHANTO...` entry also still showing — user may want that deleted too if full revocation intended. Key distinction logged: `MK2_PHANTOM_KEY` ≠ `MK2_PHANTOM_TOKEN`. | Logged in COMMS.md. Behavioral log updated. No workflow changes needed unless user confirms TOKEN also revoked. |
